...
std::complex<T2> cyl_j( const T1 _nu, const std::complex<T2> _z, bool _scaled = false, bool _flags = false )
{
    ...
    else
    {
        ...
        if ( double(floor(fnu)) == fnu )
        {
            ...
            slatec::zbesj_(&x, &y, &fnu, &kode, &n, &cjr, &cji, &nz, &ierr);
            if (_flags) _flag_zbesj_(ierr, nz);
            return T2(pow(-1., fnu)) * std::complex<T2>(cjr, cji);
        }
        else if ( double(floor(fnu)) != fnu && abs(z) == 0. )
        {
            ...
            return std::complex<T2>(dinf, dinf);
        }
        else
        {
            // Non-int negative order with abs(z)!=0.
            ...
            double tmp = fnu*M_PI;
            return ( std::complex<T2>(cjr, cji) )*T2(cos(tmp)) - ( std::complex<T2>(cyr, cyi) )*T2(sin(tmp));
        }
    }
}
template<typename T1, typename T2>
...
inline void cyl_j( const T1 _nu, const int _n, const std::complex<T2> _z, std::complex<T2> * _cyl_j, bool _scaled = false, bool _flags = false )
{
    ...
    else if ( nu_m <= 0. )
    {
        ...
        if ( double(floor(fnu)) == fnu )
        {
            ...
            {
                int tmp = n-1-i;
                _cyl_j[i] = pow(T2{-1.}, double(fnu_m+tmp)) * std::complex<T2>(cjr[tmp], cji[tmp]);
            }
        }
...
template<typename T1, typename T2>
inline std::complex<T2> cyl_y( const T1 _nu, const std::complex<T2> _z, bool _scaled = false, bool _flags = false )
{
    // Parameters
    //  _nu:     real order.
    //  _z:      complex argument.
...
template<typename T1, typename T2>
inline std::complex<T2> cyl_h1( const T1 _nu, const std::complex<T2> _z, bool _scaled = false, bool _flags = false )
{
    // Parameters
    //  _nu:     real order.
    //  _z:      complex argument.
...
template<typename T1, typename T2>
inline void cyl_h1( const T1 _nu, const int _n, const std::complex<T2> _z, std::complex<T2> * _cyl_h1, bool _scaled = false, bool _flags = false )
{
    // Parameters
    //  _nu:     real first order to be calculated.
    //  _n:      int to return an array with the orders _nu, _nu+1, ..., _nu+_n.
...
template<typename T1, typename T2>
inline void cyl_h1( const T1 _nu, const int _n, T2 _z, T2 * _cyl_h1, bool _scaled = false, bool _flags = false )
{
...
template<typename T1, typename T2>
inline std::complex<T2> cyl_h2( const T1 _nu, const std::complex<T2> _z, bool _scaled = false, bool _flags = false )
{
    // Parameters
    //  _nu:     real order.
    //  _z:      complex argument.
...
template<typename T1, typename T2>
inline void cyl_h2( const T1 _nu, const int _n, const std::complex<T2> _z, std::complex<T2> * _cyl_h2, bool _scaled = false, bool _flags = false )
{
    // Parameters
    //  _nu:     real first order to be calculated.
    //  _n:      int to return an array with the orders _nu, _nu+1, ..., _nu+_n.
...
template<typename T1, typename T2>
inline void cyl_h2( const T1 _nu, const int _n, T2 _z, T2 * _cyl_h2, bool _scaled = false, bool _flags = false )
{
...
template<typename T1, typename T2>
inline std::complex<T2> cyl_i( const T1 _nu, const std::complex<T2> _z, bool _scaled = false, bool _flags = false )
{
    // Parameters
    //  _nu:     real order.
    //  _z:      complex argument.
...
template<typename T1, typename T2>
inline std::complex<T2> cyl_k( const T1 _nu, const std::complex<T2> _z, bool _scaled = false, bool _flags = false )
{
    // Parameters
    //  _nu:     real order.
    //  _z:      complex argument.
...
inline std::complex<T1> airy_ai( const std::complex<T1> _z, bool _scaled = false, bool _flags = false )
{
    // Parameters
    //  _z:      complex argument.
...
// Airy Bi function
template<typename T1>
inline std::complex<T1> airy_bi( const std::complex<T1> _z, bool _scaled = false, bool _flags = false )
{
    // Parameters
    //  _z:      complex argument.
